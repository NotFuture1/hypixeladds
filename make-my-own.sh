...
(
  set -x
  find "$base"/src/main -type f -exec sed -i s/examplemod/"$modid"/g\;s/mod.future/"$package_name"/g {} +
  sed -i s/mod.future/"$package_name"/g\;s/skyblock/"$modid"/g "$base"/gradle.properties
